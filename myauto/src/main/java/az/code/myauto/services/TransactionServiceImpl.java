...
public class TransactionServiceImpl implements TransactionService {
    ...
    @Override
    public Transaction increaseBalance(double amount, UserData userData) throws TransactionIncorrectAmountException {
        if (amount > 0) {
    ...
    @Override
    public Transaction decreaseBalance(double amount, UserData userData, long listingId) throws TransactionIncorrectAmountException, TransactionInsufficientFundsException {
        if (amount > 0) {
            User user = userRepo.findByUsername(userData.getUsername());
